...
const getNasaImage = async () => {
    let data = await promisifiedRequest({
        uri: `https://api.nasa.gov/planetary/apod?api_key=${process.env.NASAAPI}`,
        json:true
        ...
        //   },
        //   form: {}
       // don't need the headers or form
    })
    return data.body
}
// getNasaImage()

const cocktailInfo = async (ingredient) => { // pass a parameter of userInput
    let data = await promisifiedRequest({
        url: 'https://the-cocktail-db.p.rapidapi.com/filter.php',
        qs: {i:ingredient},
        // ${ingredient}
        // req.body.ingredient - this may change depending on the data but more than likely, req.body.ingredient
        headers: {
